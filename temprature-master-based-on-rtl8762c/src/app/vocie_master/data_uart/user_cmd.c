...
/**
 * @brief Show scan dev list: filter voicele ble service
 *
 * <b>Command table define</b>
 * \code{.c}
    {
        "showdev",
        "showdev\n\r",
        "Show scan dev list: filter voicele ble service\n\r",
        cmd_showdev
    },
 * \endcode
 */
...
/**
 * @brief Read voicele ble service characteristic and descriptor value
 *
 * <b>Command table define</b>
 * \code{.c}
    {
        "voiceread",
        "voiceread [conn_id] [type] [pattern]\n\r",
        "Read voicele ble service characteristic and descriptor value\r\n\
        [type]: 0-(read v1), 1-(v3 cccd), 2-(v4 cccd)\r\n\
        [pattern]: 0-(read by handle), 1-(read by uuid)\r\n\
        sample: voiceread 0 1 0 \n\r",
        cmd_voiceread
    },
 * \endcode
 */
static T_USER_CMD_PARSE_RESULT cmd_voiceread(T_USER_CMD_PARSED_VALUE *p_parse_value)
{
    ...
    //uint8_t conn_id = p_parse_value->dw_param[0];
    bool ret = true;
    //T_VOICE_READ_TYPE read_type = (T_VOICE_READ_TYPE)p_parse_value->dw_param[1];
    /* Read by handle or UUID, 1--by UUID, 0--by handle. */
    uint8_t read_pattern = (uint8_t)p_parse_value->dw_param[2];

    if (read_pattern)
    {
        //ret = voice_client_read_by_uuid(conn_id, read_type);
    }
    else
    {
        //ret = voice_client_read_by_handle(conn_id, read_type);
    }
...
/**
 * @brief Config voicele ble service client characteristic configuration descriptor value
 *
 * <b>Command table define</b>
 * \code{.c}
    {
        "voicecccd",
        "voicecccd [conn_id] [type] [enable]\n\r",
        "Config voicele ble service client characteristic configuration descriptor value\r\n\
        [type]: 0-(v3 notify), 1-(v4 indication)\r\n\
        [enable]: 0-(disable), 1-(enable) \r\n\
        sample: voicecccd 0 1 1\n\r",
        cmd_voicecccd
    },
 * \endcode
 */
T_USER_CMD_PARSE_RESULT cmd_voicecccd(T_USER_CMD_PARSED_VALUE *p_parse_value)
{
    ...
    switch (type)
    {
    case 0:/* key Notify char notif enable/disable. */
        ret = voice_set_notify(conn_id, 1,  cccd_data);
        break;
    case 1:/* cmd Notify char indicate enable/disable. */
        ret = voice_set_notify(conn_id, 2,  cccd_data);
        break;
    case 2:/* cmd/data Notify char indicate enable/disable. */
        ret = voice_set_notify(conn_id, 3,  cccd_data);
        break;
    case 3:/* cmd/data Notify char indicate enable/disable. */
        ret = voice_set_notify(conn_id, 4,  cccd_data);
        break;
    default:
        break;
    }
...
/**
 * @brief Write voicele ble service V2 characteristic value
 *
 * <b>Command table define</b>
 * \code{.c}
    {
        "voicewritecmd",
        "voicewritecmd [conn_id] [type] [len]\n\r",
        "Write voicele ble service V2 characteristic value\r\n\
        [type]: 1-(write request), 2-(write command) \r\n\
        [len]: type=1 len range:0-270, type=2 len range: 0-(mtu-3)\r\n\
        sample: voicewritecmd 0 1 10\n\r",
        cmd_voicewritecmd
    },
 * \endcode
 */
static T_USER_CMD_PARSE_RESULT cmd_voicewritecmd(T_USER_CMD_PARSED_VALUE *p_parse_value)
{
    uint8_t conn_id = p_parse_value->dw_param[0];
    ...
    //data_uart_print("cmd_voicewritecmd,  conn_id 0x%x, write_type 0x%x\r\n", conn_id, write_type);
    //data[0] = (uint8_t)data[0];
    for (i = 0; i < 270; i++)
    {
        data[i] = i;
    }
    ...
    if (voice_ble_client_write_cmd(conn_id, length, data, write_type))
    {
        return (RESULT_SUCESS);
    }
    else
...
/**
 * @brief List voicele ble service handle cache
 *
 * <b>Command table define</b>
 * \code{.c}
    {
        "voicehdl",
        "voicehdl [conn_id]\n\r",
        "List voicele ble service handle cache\n\r",
        cmd_voicehdl
    },
 * \endcode
 *///#include "trace.h"
T_USER_CMD_PARSE_RESULT cmd_voicehdl(T_USER_CMD_PARSED_VALUE *p_parse_value)
{
    uint8_t conn_id = p_parse_value->dw_param[0];
    ...
    if (ret)
    {
        for (hdl_idx = HDL_VOICE_SRV_START; hdl_idx < HDL_VOICE_CACHE_LEN; hdl_idx++)
        {
            //data_uart_print("-->Index %d -- Handle 0x%x\r\n", hdl_idx, hdl_cache[hdl_idx]);
            APP_PRINT_INFO1("[Temp] handle >> %#x",  hdl_cache[hdl_idx]);
        }
        return (RESULT_SUCESS);
    }
    else
...
const T_USER_CMD_TABLE_ENTRY user_cmd_table[] =
{
    ...
    },
    {
        "voicewritecmd",
        "voicewritecmd [conn_id] [type] [len]\n\r",
        "Write voicele ble service cmd characteristic value\r\n\
        [type]: 1-(write request), 2-(write command) \r\n\
        [len]: type=1 len range:0-270, type=2 len range: 0-(mtu-3)\r\n\
        sample: voicewritecmd 0 1 10\n\r",
        cmd_voicewritecmd
