...
T_CLIENT_ID   voice_client_id;         /**< battery service client id*/
...
DATA_RAM_FUNCTION T_APP_GLOBAL_DATA app_global_data = {0};
/*============================================================================*
 *                              Functions
 *============================================================================*/
...
//                            bt_addr[0]);
            ...
            app_global_data.device_ble_status = DEVICE_STATUS_IDLE;
        }
    }
    ...
    if (gap_dev_state.gap_scan_state != new_state.gap_scan_state)
    {
        if (new_state.gap_scan_state == GAP_SCAN_STATE_IDLE)
        {
            ...
            LED_BLINK_EXIT(LED_1, LED_TYPE_BLINK_PAIR_SCAN);
        ...
        else if (new_state.gap_scan_state == GAP_SCAN_STATE_SCANNING)
        {
            ...
            LED_BLINK(LED_1, LED_TYPE_BLINK_PAIR_SCAN, 0);
            //data_uart_print("GAP scan start\r\n");
            app_global_data.device_ble_status = DEVICE_STATUS_SCAN;
        }
    }
...
void app_handle_conn_state_evt(uint8_t conn_id, T_GAP_CONN_STATE new_state, uint16_t disc_cause)
{
    ...
    case GAP_CONN_STATE_DISCONNECTED:
        {
            LED_BLINK_EXIT(LED_1, LED_TYPE_BLINK_PAIR_SCAN);
            LED_BLINK_EXIT(LED_1, LED_TYPE_ON);
            APP_PRINT_INFO0("[GAP_CONN_STATE_DISCONNECTED] ");
            app_global_data.device_ble_status = DEVICE_STATUS_IDLE;
            ...
            APP_PRINT_INFO1("Disconnect conn_id %d\r\n", conn_id);
...
T_APP_RESULT app_gap_callback(uint8_t cb_type, void *p_cb_data)
{
    ...
    case GAP_MSG_LE_SCAN_INFO:
        ...
        //if (filter_scan_info_by_uuid(GATT_UUID_VOICELE_PROFILE, p_data->p_le_scan_info))
        if (voice_filter_device_mac(p_data->p_le_scan_info->bd_addr))
        {
            APP_PRINT_INFO0("[Temp] Found temp ble device!");
            link_mgr_add_device(p_data->p_le_scan_info->bd_addr, p_data->p_le_scan_info->remote_addr_type);
            app_global_data.is_scan_device_hit = true;
        }
...
void app_discov_services(uint8_t conn_id, bool start)
{
    ...
    if ((app_link_table[conn_id].discovered_flags & APP_DISCOV_VOICE_FLAG) == 0)
    {
        if (voice_start_discovery(conn_id) == false)
        {
            APP_PRINT_ERROR1("app_discov_services: discover voice failed conn_id %d", conn_id);
        }
    }
...
T_APP_RESULT app_client_callback(T_CLIENT_ID client_id, uint8_t conn_id, void *p_data)
{
    ...
    else if (client_id == voice_client_id)
    {
        T_VOICE_CLIENT_CB_DATA *p_voice_client_cb_data = (T_VOICE_CLIENT_CB_DATA *)p_data;
        uint16_t value_size;
        uint8_t *p_value;

        switch (p_voice_client_cb_data->cb_type)
        {
        case VOICE_CLIENT_CB_TYPE_DISC_STATE:
            switch (p_voice_client_cb_data->cb_content.disc_state)
            {
            case DISC_VOICE_DONE:
                /* Discovery Simple BLE service procedure successfully done. */
                app_link_table[conn_id].discovered_flags |= APP_DISCOV_VOICE_FLAG;
                app_link_table[conn_id].srv_found_flags |= APP_DISCOV_VOICE_FLAG;
                app_discov_services(conn_id, false);
                APP_PRINT_INFO0("app_client_callback: discover temperature procedure done.");
                app_global_data.device_ble_status = DEVICE_STATUS_SERVICE_DISCOV_DONE;
                break;
            case DISC_VOICE_FAILED:
                /* Discovery Request failed. */
                app_link_table[conn_id].discovered_flags |= APP_DISCOV_VOICE_FLAG;
                app_discov_services(conn_id, false);
            ...
            break;
        case VOICE_CLIENT_CB_TYPE_READ_RESULT:
            switch (p_voice_client_cb_data->cb_content.read_result.type)
            {
...
//                    APP_PRINT_INFO2("VOICE_READ_V1_READ: value_size %d, value %b",
//                                    value_size, TRACE_BINARY(value_size, p_value));
//                }
//                else
//                {
//                    APP_PRINT_ERROR1("VOICE_READ_V1_READ: failed cause 0x%x",
...
//                if (p_simp_client_cb_data->cb_content.read_result.cause == GAP_SUCCESS)
//                {
//                    APP_PRINT_INFO1("VOICE_READ_V3_NOTIFY_CCCD: notify %d",
//                                    p_simp_client_cb_data->cb_content.read_result.data.v3_notify_cccd);
//                }
//                else
//                {
//                    APP_PRINT_ERROR1("VOICE_READ_V3_NOTIFY_CCCD: failed cause 0x%x",
//                                     p_simp_client_cb_data->cb_content.read_result.cause);
//                };
//                break;
...
//                if (p_simp_client_cb_data->cb_content.read_result.cause == GAP_SUCCESS)
//                {
//                    APP_PRINT_INFO1("VOICE_READ_V4_INDICATE_CCCD: indicate %d",
//                                    p_simp_client_cb_data->cb_content.read_result.data.v4_indicate_cccd);
//                }
//                else
//                {
//                    APP_PRINT_ERROR1("VOICE_READ_V4_INDICATE_CCCD: failed cause 0x%x",
//                                     p_simp_client_cb_data->cb_content.read_result.cause);
//                };
//                break;
            ...
            break;
        case VOICE_CLIENT_CB_TYPE_WRITE_RESULT:
            switch (p_voice_client_cb_data->cb_content.write_result.type)
            {
            case VOICE_WRITE_DATA:
#if RCU_VOICE_EN
            ...
            break;
        case VOICE_CLIENT_CB_TYPE_NOTIF_IND_RESULT:
            {
                switch (p_voice_client_cb_data->cb_content.notify_data.dataType)
                {
                case HDL_VOICE_TYPE_DATA:
                    value_size = p_voice_client_cb_data->cb_content.notify_data.bufLength;
                    p_value = p_voice_client_cb_data->cb_content.notify_data.pDataBuf;

                    //data_uart_send(value_size, p_value);
#if FEATURE_SUPPORT_AUDIO_DOWN_STREAMING
                    app_handle_hids_ads_write_char_tx(p_value, value_size);
#endif
                    break;
                case HDL_VOICE_TYPE_KEY:
                    value_size = p_voice_client_cb_data->cb_content.notify_data.bufLength;
                    p_value = p_voice_client_cb_data->cb_content.notify_data.pDataBuf;
                    APP_PRINT_INFO2("[Voice] HDL_VOICE_TYPE_KEY: value_size %d, value %b",
                                    value_size, TRACE_BINARY(value_size, p_value));
#if FEATURE_SUPPORT_AUDIO_DOWN_STREAMING
                    if ((value_size == 8) && (*(p_value + 2) == 0x3e))
                    {
                        audio_handle_start();
                    }
                    else if ((value_size == 8) && (*(p_value + 2) == 0x00))
                    {
                        audio_handle_stop();
                    }
#endif
                    break;
                case HDL_VOICE_TYPE_CMD:
                    value_size = p_voice_client_cb_data->cb_content.notify_data.bufLength;
                    p_value = p_voice_client_cb_data->cb_content.notify_data.pDataBuf;
                    APP_PRINT_INFO2("[Voice] HDL_VOICE_TYPE_CMD: value_size %d, value %b",
                                    value_size, TRACE_BINARY(value_size, p_value));
                    break;
                case HDL_TEMP_TYPE_DATA:
                    value_size = p_voice_client_cb_data->cb_content.notify_data.bufLength;
                    p_value = p_voice_client_cb_data->cb_content.notify_data.pDataBuf;
                    APP_PRINT_INFO2("[Voice] HDL_VOICE_TYPE_CMD: value_size %d, value %b",
                                    value_size, TRACE_BINARY(value_size, p_value));
