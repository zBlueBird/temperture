...
void UART0_Handler(void)
{
    ...
    case UART_INT_ID_RX_LEVEL_REACH:
        UART_ReceiveData(UART, &rx_char, 1);
...
void data_uart_send(uint8_t block_size, uint8_t *p_buf)
{
    uint8_t blk_cnt, remainder;

    blk_cnt = (block_size) / UART_TX_FIFO_SIZE;
    remainder = (block_size) % UART_TX_FIFO_SIZE;
