                        ...
                        uint32_t/*void*/ *p_void)
{
    ...
    {
        T_IMG_CTRL_HEADER_FORMAT *p_header = (T_IMG_CTRL_HEADER_FORMAT *) p_void;
        p_header->ctrl_flag.flag_value.not_ready = 0x1; /*access ram addr, make sure image is not ready*/
...
bool slient_dfu_check_crc(T_IMG_CTRL_HEADER_FORMAT *p_header)
{
    ...
    DFU_PRINT_INFO2("<===dfu_check_crc checksum16: 0x%04x, expect: 0x%04x\r\n", calced_crc16,
                    img_crc16);
...
bool slient_dfu_check_sha256(T_IMG_CTRL_HEADER_FORMAT *p_header)
{
    ...
    DFU_PRINT_INFO2("dfu_check_sha256: Calculating result: %b %b", TRACE_BINARY(SHA256_LENGTH,
                                                                                sha256sum), TRACE_BINARY(SHA256_LENGTH, sha256img));
...
bool silent_check_image_chksum(T_IMG_CTRL_HEADER_FORMAT *p_header)
{
    ...
    {
        crc16 = r_data >> 16;
        DFU_PRINT_INFO1("crc16=0x%x", crc16);
