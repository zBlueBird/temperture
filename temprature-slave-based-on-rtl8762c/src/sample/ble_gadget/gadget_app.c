...
/** @addtogroup  PERIPH_SEVER_CALLBACK Profile Server Callback Event Handler
    ...
    * @{
    */
T_SERVER_ID ams_srv_id; /**< Simple ble service id*/
static ble_ama_t m_ama;
static bool srv_dis_done = false;
...
T_APP_RESULT app_profile_callback(T_SERVER_ID service_id, void *p_data)
{
    ...
    else  if (service_id == ams_srv_id)
    {
        T_GADGET_CALLBACK_DATA *p_gadget_cb_data = (T_GADGET_CALLBACK_DATA *)p_data;
        switch (p_gadget_cb_data->msg_type)
        {
        case SERVICE_CALLBACK_TYPE_INDIFICATION_NOTIFICATION:
            {
                if (srv_dis_done)
                {
                    on_subscription(&m_ama, p_gadget_cb_data->msg_data.notification_indification_index);
                    srv_dis_done = false;
                }
            }
            break;
        ...
        case SERVICE_CALLBACK_TYPE_READ_CHAR_VALUE:
            {
                if (p_gadget_cb_data->msg_data.read_value_index == GADGET_READ)
                {
                    uint8_t value[2] = {0x01, 0x02};
                    APP_PRINT_INFO0("GADGET_READ");
                    ama_ble_service_set_parameter(GADGET_BLE_SERVICE_PARAM_READ_CHAR_VAL, 2, &value);
                }
            }
            break;
        case SERVICE_CALLBACK_TYPE_WRITE_CHAR_VALUE:
            {
                DBG_DIRECT("GADGET, WRITE_RECEIVED");
                switch (p_gadget_cb_data->msg_data.write.opcode)
                {
                case GADGET_WRITE:
                    {
                        APP_PRINT_INFO2("GADGET_WRITE: write type %d, len %d", p_gadget_cb_data->msg_data.write.write_type,
                                        p_gadget_cb_data->msg_data.write.len);
                        on_data_received(&m_ama, p_gadget_cb_data);
                    }
                    break;
                default:
                    break;
...
uint8_t ble_ama_init(ble_ama_t *p_ama)
{
    uint8_t err_code = 0;
    if (p_ama == NULL)
    {
    ...
    m_ama.conn_handle               = 0;
    m_ama.mtu                       = 300;
    memset(&p_ama->address, 0, sizeof(AMAAddress));

    // TODO: Set the AMAAddress to the current value of p_ama to
    // be able to retrieve it in AMATransport_SendDataCallback
    // Need to find a better way to do this since AMATransport does
    // not accept a generic arg that it can pass to us again in
    // the callback.
    memcpy(p_ama->address.raw, (char *)&p_ama, sizeof(intptr_t));

    return err_code;
}

void ama_struct_init(void)
{
    ble_ama_init(&m_ama);
}
/** @} */ /* End of group PERIPH_SEVER_CALLBACK */
