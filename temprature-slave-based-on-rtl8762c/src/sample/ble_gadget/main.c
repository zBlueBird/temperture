...
#define DEFAULT_ADVERTISING_INTERVAL_MIN            800
/** @brief  Default maximum advertising interval */
#define DEFAULT_ADVERTISING_INTERVAL_MAX            800

#define SCAN_REPSONSE_DATA_LEN                      16
...
static char scan_rsp_data[] =
{
    /* Local name */
    0x0F,             /* length */
    GAP_ADTYPE_LOCAL_NAME_COMPLETE,
    'R', 'T', 'K', 'G', 'a', 'd', 'g', 'e', 't', '_', 'X', 'X', 'X', 'X', '\0',
};
...
static const uint8_t adv_data[] =
{
    ...
    LO_WORD(AMA_SERVICE_UUID),
    HI_WORD(AMA_SERVICE_UUID),
    /* Local name */
    0x17,             /* length */
...
void app_le_gap_init(void)
{
    ...
    char device_name[GAP_DEVICE_NAME_LEN] = "RTKGadget_XXXX";
    size_t device_name_len = strlen(device_name);
    sprintf(&device_name[device_name_len - 4], "%02x%02x", bt_addr[1], bt_addr[0]);
    sprintf(&scan_rsp_data[SCAN_REPSONSE_DATA_LEN - 4], "%02x%02x", bt_addr[1], bt_addr[0]);
    gadget_set_name(device_name);
    ...
    uint16_t auth_flags = GAP_AUTHEN_BIT_BONDING_FLAG | GAP_AUTHEN_BIT_SC_ONLY_FLAG;;
    ...
    uint16_t auth_sec_req_flags = GAP_AUTHEN_BIT_BONDING_FLAG | GAP_AUTHEN_BIT_SC_ONLY_FLAG;
    ...
    le_adv_set_param(GAP_PARAM_SCAN_RSP_DATA, sizeof(scan_rsp_data) - 1, (void *)scan_rsp_data);
...
void app_le_profile_init(void)
{
    ama_struct_init();
    server_init(2);
    ams_srv_id = ama_ble_service_add_service(app_profile_callback);
...
void io_dlps_enter_cb(void)
{
    ...
    Pad_Config(LED_PIN, PAD_SW_MODE, PAD_IS_PWRON, PAD_PULL_NONE, PAD_OUT_ENABLE, PAD_OUT_LOW);
...
{
    Pad_Config(LPN_BUTTON, PAD_PINMUX_MODE, PAD_IS_PWRON, PAD_PULL_UP, PAD_OUT_DISABLE, PAD_OUT_LOW);
