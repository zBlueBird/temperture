...
const T_ATTRIB_APPL gattOtaServiceTable[] =
{
    ...
    /* <<Characteristic3>>, .. 11,ota pack img version*/            //-------------------------ota pack image version  9
    {
        ATTRIB_FLAG_VALUE_INCL,                     /* wFlags */
        {   /* bTypeValue */
            LO_WORD(GATT_UUID_CHARACTERISTIC),
    ...
    case BLE_SERVICE_CHAR_IMAGE_VERSION_INDEX:
        {
            ...
            PROFILE_PRINT_INFO1("ota header addr = %x", image_addr);
            ...
            PROFILE_PRINT_INFO4("OTA Header V_0x%X, Secure Boot V_0x%X, Patch V_0x%X, APP V_0x%X",
