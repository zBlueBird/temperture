...
{
#if 0
    /*check voice adc is working, if yes return previous value*/
    if (is_VoiceWorking())
    {
        return (uint32_t)vbat_calibrate_voltage(rcu_vbat_value);
    }
#endif
...
#if (!BAT_LPC_EN)
            /*if rcu system not in low power mode, check battery level by adc when lpc disable*/
            else
            {
                ...
                /*cllect battery value*/
                /*T_IO_MSG bee_io_msg = {0};
                bee_io_msg.type = IO_MSG_TYPE_BAT_LPC;
                if (false == app_send_msg_to_apptask(&bee_io_msg))
                {
                    APP_PRINT_ERROR0("[BAT] ADC send IO_MSG_TYPE_LPC message failed!");
                    return;
                }*/
                battery_get_value(&level, &value);
                if (value < BAT_LOW_POWER_THRESHOLD)
